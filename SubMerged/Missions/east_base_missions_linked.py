...
def EAST3(br: BaseRobot):
    ...
    br.settings(turn_acceleration=200)
    br.turn(24, Stop.BRAKE)
    br.back_attachment.run_angle(1000,-815, Stop.BRAKE,)
    br.straight(-50, Stop.BRAKE)
    while not (77 < br.hub.imu.heading() < 81):
        br.drive(0, (79 - br.hub.imu.heading()) * 3)
    br.stop()
    br.turn(-50, Stop.BRAKE)
    while not (-42 < br.hub.imu.heading() < -38):
        br.drive(0, (-40 - br.hub.imu.heading()) * 3)
    br.stop()
    br.straight(330)
    br.back_attachment.run_angle(1000,150, Stop.BRAKE, wait=True)
    br.turn(-10, Stop.BRAKE)
    br.straight(22, Stop.BRAKE)
    while not (-66 < br.hub.imu.heading() < -62):
        br.drive(0, (-64 - br.hub.imu.heading()) * 3)
    br.stop()
    br.turn_attachment(br.back_attachment, 1200, 1100, wait_time=1700)
    br.back_attachment.run_angle(1200, -800)
    br.straight(-100, Stop.BRAKE)
    ...
    # br.turn(30, Stop.BRAKE)
    # print(br.hub.imu.heading())
    # while not (-27 < br.hub.imu.heading() < -23):
    #     br.drive(0, (-25 - br.hub.imu.heading()) * 3)
    # br.stop()
    # br.settings(straight_speed=100)
    # br.straight(150, Stop.BRAKE)
    # br.turn_attachment(br.back_attachment, 1000, 500, wait_time=1700)
    """
    br.straight(-400, Stop.BRAKE)
    br.turn(20, Stop.BRAKE)
    br.straight(-500, Stop.BRAKE)
